...
uint16_t cLEDMatrix::Size() {
  return NUM_LEDS;
}
uint8_t cLEDMatrix::Width() {
  return MATRIX_WIDTH;
}
uint8_t cLEDMatrix::Height() {
  return MATRIX_HEIGHT;
}
uint16_t cLEDMatrix::GetXY(int8_t x, int8_t y) {
...
void cLEDMatrix::HorizontalMirror() {
...
void cLEDMatrix::DrawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, CRGB Col) {
  int16_t dx = x1 - x0;
  int16_t dy = y1 - y0;
  if (abs(dx) >= abs(dy)) {
...
void cLEDMatrix::DrawRectangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, CRGB Col) {
...
void cLEDMatrix::DrawCircle(int16_t xc, int16_t yc, uint16_t r, CRGB Col) {
  int16_t x = -r;
  int16_t y = 0;
  int16_t e = 2 - (2 * r);
  do {
...
void cLEDMatrix::DrawFilledRectangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, CRGB Col) {
  int16_t y = min(y0, y1);
  for (int16_t c = abs(y1 - y0); c >= 0; --c, ++y)
    DrawLine(x0, y, x1, y, Col);
}
void cLEDMatrix::DrawFilledCircle(int16_t xc, int16_t yc, uint16_t r, CRGB Col) {
  int16_t x = r;
  int16_t y = 0;
  int16_t e = 1 - x;
  while (x >= y) {
